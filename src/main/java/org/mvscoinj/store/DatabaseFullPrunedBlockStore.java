...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See mvscoinj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link org.mvscoinj.core.NetworkParameters}.
     ...
     *
     * <p>The balance {@link org.mvscoinj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
