...
import static org.mvscoinj.core.Utils.*;
 ...
 * </ul>
 */
public class MvscoinSerializer extends MessageSerializer {
    private static final Logger log = LoggerFactory.getLogger(MvscoinSerializer.class);
    ...
    /**
     * Constructs a MvscoinSerializer with the given behavior.
     *
     * @param params      networkParams used to create Messages instances and termining packetMagic
     * @param parseRetain retain the backing byte array of a message for fast reserialization.
     */
    public MvscoinSerializer(NetworkParameters params, boolean parseRetain) {
        this.params = params;
    ...
    public void serialize(String name, byte[] message, OutputStream out) throws IOException {
        ...
        uint32ToByteArrayLE(params.getPacketMagic(), header, 0);
    ...
    public void serialize(Message message, OutputStream out) throws IOException {
        ...
        if (name == null) {
            throw new Error("MvscoinSerializer doesn't currently know how to serialize " + message.getClass());
        }
     ...
     * {@link MvscoinSerializer#deserializeHeader}.
     */
    @Override
    ...
    public static class BitcoinPacketHeader {
         ...
         */
        public static final int HEADER_LENGTH = COMMAND_LEN + 4 + 4 + 4;
        ...
        public BitcoinPacketHeader(ByteBuffer in) throws ProtocolException, BufferUnderflowException {
            ...
            for (; cursor < COMMAND_LEN; cursor++) ;
            byte[] commandBytes = new byte[cursor];
            System.arraycopy(header, 4, commandBytes, 0, cursor);
            command = Utils.toString(commandBytes, "US-ASCII").trim();
            cursor = COMMAND_LEN;

            size = (int) readUint32(header, cursor+4);
            cursor += 8;
