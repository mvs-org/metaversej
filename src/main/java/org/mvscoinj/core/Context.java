 ...
 * mvscoinj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating mvscoinj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use mvscoinj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a mvscoinj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using mvscoinj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing mvscoinj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.mvscoinj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.mvscoinj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.mvscoinj.core.NetworkParameters} specified when this context was (auto) created. The
